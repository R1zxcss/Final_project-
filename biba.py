...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        btn_open_dialog = tk.Button(
            toolbar, bg="#d7d8e0", bd=0, image=self.add_img, 
            command=self.open_dialog
        )
        btn_open_dialog.pack(side=tk.LEFT)

        self.tree = ttk.Treeview(
            self, columns=("ID", "name", "tel", "email", "wages"), 
    ...
    def update_records(self, name, tel, email, wages):
        ...
        (name, tel, email, wages, self.tree.set
...
class DB:
    ...
    def insert_data(self, name, tel, email, wages):
        self.cursor.execute(
            """INSERT INTO db(name, tel, email, wages) VALUES(?, ?, ?, ?)""", 
            (name, tel, email, wages)
        )
        self.conn.commit()
